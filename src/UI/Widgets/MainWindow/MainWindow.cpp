...
void CMainWindow::Render()
{
	...
	if (ImGui::Begin("Nexus", (bool*)0, Flags | (this->IsHandleHeld ? 0 : ImGuiWindowFlags_NoMove)))
	{
		...
		if (ImGui::BeginChild("Footer", ImVec2(contentWidth, footerHeight), false, ImGuiWindowFlags_NoBackground))
		{
			...
			ImGui::SetCursorPos(ImVec2(((contentWidth - ImGui::CalcTextSize(reinterpret_cast<const char*>(u8"© 2021 - 2026 Raidcore")).x) / 2) - sidebarWidth, (footerHeight - ImGui::GetFontSize()) / 2));
			ImGui::Text(reinterpret_cast<const char*>(u8"© 2021 - 2026 Raidcore"));
		}
		ImGui::EndChild();
