...
{
	/*static*/ RefCleanerContext* RefCleanerContext::Get()
	{
		static RefCleanerContext s_Context{};
		return &s_Context;
	}

	void RefCleanerContext::Register(std::string aComponentName, IRefCleaner* aComponent)
	{
	...
	void RefCleanerContext::Deregister(IRefCleaner* aComponent)
	{
		const std::lock_guard<std::mutex> lock(this->Mutex);
	...
	std::string RefCleanerContext::CleanupRefs(void* aStartAddress, void* aEndAddress)
	{
		std::string result;
